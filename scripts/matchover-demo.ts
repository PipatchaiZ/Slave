// Auto-plays a full single-round match to match_over, then freezes so the
// results screen (role icons in front of player names) can be screenshotted.
...
async function main() {
  ...
  socks.forEach((s) =>
    s.on(EV.state, (v: GameView) => {
      if (v.phase !== 'playing') return; // single round: no exchange; stop at match_over
      const mv = move(v);
  ...
  await sleep(4000); // let the bots auto-play the single round to match_over
  writeFileSync('scripts/.creds.json', JSON.stringify({ code: c.roomCode, creds }, null, 2));
  console.log('seeded match, room', c.roomCode);
