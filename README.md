...
เกมไพ่สลาฟ 3-6 คนแบบออนไลน์เรียลไทม์ สไตล์ 8-bit ไม่มีฐานข้อมูล (state อยู่ใน memory)
...
เปิดหลายแท็บ/อุปกรณ์เพื่อจำลองผู้เล่นหลายคน: คนแรกกด "สร้างห้อง" ได้โค้ด 5 หลัก คนอื่นกรอกโค้ดเข้าห้อง host กดเริ่มเมื่อมี 3-6 คน
...
**กันห้องหายตอนรีสตาร์ท (ออปชัน):** ตั้ง env `REDIS_URL` เป็น URL ของ Redis (เช่น [Upstash](https://upstash.com) ฟรี — `rediss://...`) แล้วเซิร์ฟเวอร์จะเซฟ/กู้สเตตของทุกห้องอัตโนมัติ ผู้เล่นกลับเข้าห้องเดิมได้ด้วย session เดิม ถ้าไม่ตั้งค่า → เก็บใน RAM อย่างเดียว (ดีฟอลต์เดิม)

> ⚠️ ฟรีทเทียร์มัก "หลับ" เมื่อไม่มีคนเล่น → ปลุกครั้งแรกช้า ~30 วิ ถ้าไม่ได้ตั้ง `REDIS_URL` เกมที่ค้างอยู่จะหายตอนหลับ/รีสตาร์ท (state อยู่ใน RAM)
